...
async function handleDelete(id: number) {
  try {
    await ElMessageBox.confirm('确认删除该教师？', '警告', { type: 'warning' })
    await http.delete(`/api/admin/teachers/${id}`)
...
async function submit() {
  ...
  await formRef.value.validate(async (valid: boolean) => {
    if (valid) {
      try {
        if (form.value.id) {
          await http.put(`/api/admin/teachers/${form.value.id}`, form.value)
        } else {
          await http.post('/api/admin/teachers', form.value)
        }
